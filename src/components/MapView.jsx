...
const MapView = ({
  ...
  useEffect(() => {
    ...
    const upBoundaryLayer = new ImageLayer({
      source: new ImageWMS({
        url: "http://localhost:8080/geoserver/wms",   // ✅ FIXED
  ...
  useEffect(() => {
    ...
    const url = `http://localhost:8080/geoserver/ows?service=WFS&request=GetFeature&typeName=${encodeURIComponent(selectedLayer)}&outputFormat=application/json`;
